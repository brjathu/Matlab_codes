...
//<<<<<<<<<<<<<<<<<<< biblioteca <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void ssimcor(const VETOR<double>& Y, double y2, double my, IMGCOR& cy, const VETOR<double>& X, double x2, double mx, IMGCOR& cx,
...
//<<<<<<<<<<<<<<<<<<<< Cifi <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void cissmatch(I3DCOR& a, int l, int c, const VETOR<double>& X, double mX, IMGCOR& cx,
                          double& coefstr, double & coefcor, double& coef)
{ VETOR<double> Y(X.n());
...
{ using namespace CirateC;
  if (argc!=2 && argc!=5) {
    printf("CirateCS: Pyramidal Ciratefi for color images v1.06\n");
